...
 when you only need to use the function one or twice, you can call the `function/dataset` through the notation `package::functionname`
...
\#`{r} #m1 <- matrix(rep(10, 9), 3, 3) m2 <- cbind (c(1,2,3), c(4, 5, 6)) m3 <- cbind (m1[,1], m2[2,]) m4 <- cbind (m1[,1], m2[,2]) #`
